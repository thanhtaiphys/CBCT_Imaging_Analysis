...
"""
NTCP Analysis Program for Left Breast after MV-CBCT (5MU and 10MU)

This program analyzes CBCT (Cone Beam Computed Tomography) data for breast cancer patients,
...
2. Calculates NTCP for Organs at Risk (OARs)
...
Author: thanhtaiphys
Created: 2025-04-07 15:31:03 UTC
Last Modified: 2025-04-17 11:18:00 UTC
"""
...
# ROI definitions and NTCP model parameters
...
def main():
    ...
    df.to_csv("CBCT_Breast_Left_Summary_WithStats.csv", index=False)
    print("\nSummary Statistics:")
    print(df.head())
    ...
    stats_df.to_csv("CBCT_Breast_Left_Statistics.csv", index=False)
    ...
    create_boxplot(
        data=df,
        y_variable="NTCP",
        title="Comparison of NTCP Between 5 MU and 10 MU Imaging Plans for Breast (Left)",
        ylabel="NTCP (%)",
        output_filename="CBCT_NTCP_Boxplot_WithStats_for_Breast_Left.png"
    )

    create_boxplot(
        data=df,
        y_variable="Mean Dose",
        title="Comparison of Mean CBCT Dose Delivered by 5 MU and 10 MU Imaging Plans for Breast (Left)",
        ylabel="Mean Dose (Gy)",
        output_filename="CBCT_Dose_Boxplot_WithStats_for_Breast_Left.png"
    )
