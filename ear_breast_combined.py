...
"""
Secondary Cancer Risk Analysis for Combined Breast CBCT Data

This script calculates Excess Absolute Risk (EAR) for selected Organs at Risk (ROIs)
including contralateral breast and lungs, using combined data from left and right
breast cancer patients. The analysis includes age-stratified results and
...
Created: 14/04/2025
Last Modified: 2025-04-17 11:28:47
"""
...
# Model parameters (Schneider's model)
MODEL_PARAMS = {
    "alpha_prime": 0.085,
    "gamma_e": -0.3,
    "gamma_a": 1.0,
...
def load_and_merge_data(left_file: str, right_file: str, patient_info_file: str) -> pd.DataFrame:
    """
    Load and merge breast CBCT data with patient information.

    Args:
        left_file (str): Path to left breast CBCT data
        right_file (str): Path to right breast CBCT data
    ...
    cbct_df = pd.concat([left_df, right_df], ignore_index=True)
...
def create_log_scale_boxplot(data: pd.DataFrame, valid_rois: list, 
    ...
    Args:
        ...
        valid_rois (list): List of ROIs to include in the plot
        output_file (str): Output file path for saving the plot
    """
    filtered_df = data[data['ROI'].isin(valid_rois)].dropna(subset=['EAR'])
    
    plt.figure(figsize=PLOT_CONFIG["figsize_regular"])
    sns.boxplot(data=filtered_df, x='ROI', y='EAR', hue='Plan')
    
    plt.yscale('log')
    plt.title('EAR Comparison (5MU vs 10MU) – Log Scale', 
...
def calculate_summary_statistics(data: pd.DataFrame, valid_rois: list) -> pd.DataFrame:
    """
    Calculate extended summary statistics for EAR values.

    Args:
        data (pd.DataFrame): Input data containing EAR values
        valid_rois (list): List of ROIs to include in analysis

    Returns:
        pd.DataFrame: Summary statistics including mean, median, IQR, etc.
    """
    filtered_df = data[data['ROI'].isin(valid_rois)]
...
def analyze_age_groups(data: pd.DataFrame, valid_rois: list) -> tuple:
    """
    ...
    age_group_summary = data[data['ROI'].isin(valid_rois)] \
    ...
    plt.title('Mean EAR by Age Group and ROI', 
              fontsize=PLOT_CONFIG["fontsize"]["title"])
...
def main():
    ...
    merged_df = load_and_merge_data(
        ...
        "CBCT_Breast_Right_Summary_WithStats.csv",
        "Patient_Info.csv"
    )
    ...
    create_log_scale_boxplot(
        merged_df, 
        list(EAR0_DICT.keys()),
        "CBCT_EAR_Boxplot_BreastCombined_LogScale.png"
    )
    
    # Export patient-level EAR data
    detailed_ear = merged_df[['PatientID', 'Age_at_Treatment', 'Plan', 'ROI', 'EAR']].dropna()
    detailed_ear.to_csv("CBCT_EAR_Detail_ByPatient.csv", index=False)
    
    # Calculate and export summary statistics
    summary_stats = calculate_summary_statistics(merged_df, list(EAR0_DICT.keys()))
    summary_stats.to_csv("CBCT_EAR_Summary_BreastCombined_WithMedian_IQR.csv", index=False)
    
    # Perform age-stratified analysis
    age_group_summary, age_plot = analyze_age_groups(merged_df, list(EAR0_DICT.keys()))
    age_group_summary.to_csv("CBCT_EAR_AgeGroup_Summary.csv", index=False)
    age_plot.savefig("CBCT_EAR_Barplot_ByAgeGroup.png", dpi=PLOT_CONFIG["dpi"])
    plt.close()
