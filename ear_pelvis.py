...
"""
Secondary Cancer Risk Analysis for Pelvis CBCT Data

This script calculates Excess Absolute Risk (EAR) for selected Organs at Risk (ROIs)
in the pelvic region using CBCT (Cone Beam Computed Tomography) data. The analysis
includes age-stratified results and log-scale visualizations.
...
Created: 15/04/2025
Last Modified: 2025-04-17 11:30:56
"""
...
def create_log_scale_boxplot(data: pd.DataFrame, roi_list: list, 
    ...
    plt.yscale('log')
    plt.title('EAR Comparison (5MU vs 10MU) – Pelvis ROI (Log Scale)', 
...
def analyze_age_groups(data: pd.DataFrame, roi_list: list) -> tuple:
    ...
    plt.title('Mean EAR by Age Group and ROI (Pelvis CBCT)', 
              fontsize=PLOT_CONFIG["fontsize"]["title"])
...
def main():
    ...
    merged_df = load_and_merge_data(
        "CBCT_Pelvis_Summary_WithStats.csv",
        "Patient_Info.csv"
    )
    ...
    create_log_scale_boxplot(
        merged_df, 
        roi_list,
        "CBCT_EAR_Boxplot_Pelvis_LogScale.png"
    )
    
    # Calculate and export summary statistics
    summary_stats = calculate_summary_statistics(merged_df, roi_list)
    summary_stats.to_csv("CBCT_EAR_Summary_Pelvis_WithMedian_IQR.csv", index=False)
    
    # Perform and export age-stratified analysis
    age_group_summary, age_plot = analyze_age_groups(merged_df, roi_list)
    age_group_summary.to_csv("CBCT_EAR_AgeGroup_Summary_Pelvis.csv", index=False)
    age_plot.savefig("CBCT_EAR_Barplot_ByAgeGroup_Pelvis.png", dpi=PLOT_CONFIG["dpi"])
    plt.close()
    ...
    filtered_df[['PatientID', 'Age_at_Treatment', 'Plan', 'ROI', 'EAR']].to_csv(
        "CBCT_EAR_Detail_ByPatient_Pelvis.csv", index=False
    )
